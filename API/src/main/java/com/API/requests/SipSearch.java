...
public class SipSearch 
{
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			parentJsonObject.put("echo",generateGetforSIPSearchDefaultGroupEchoJSON(workbook, "DT_EchoEntity", scenarioID, excelOperation));
			...
			Reporter.log("<b><font size=4 color=green> SIP Search</font></b>");
	...
	/**
	 * Generate getfor SIP search default group echo JSON.
	 *
	 * @param workbook the workbook
	 * @param sheetName the sheet name
	 * @param scenarioID the scenario ID
	 ...
	 * @return the object
	 */
	private Object generateGetforSIPSearchDefaultGroupEchoJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		...
		for(LinkedHashMap<String, String> json:jsonMap) {
			...
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_SIPSearch", scenarioID, excelOperation));
	...
	private Object generateGetSymbolforDefaultGroupDataJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		JSONObject getGroupforLiveUserRequestJsonObject=new JSONObject();
			...
			for(LinkedHashMap<String, String> jsons:jsonMaps) 
			{
				...
				{
					getGroupforLiveUserRequestJsonObject.put("searchString", jsons.get("searchString"));
	 ...
	 */
	public void GetSipSearch(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.sip_Search.getUrl();
			...
			if(infoid.equalsIgnoreCase("0")) {
				...
				String infomsg=  ((JsonObject) jsonObject.get("response")).get("	").toString().replace("\"", "");
