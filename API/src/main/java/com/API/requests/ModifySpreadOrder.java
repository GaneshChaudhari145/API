...
/**
 * The Class ModifySpreadOrder.
 */
public class ModifySpreadOrder {
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			Reporter.log("<b><font size=4 color=green>ModifySpreadOrder</font></b>");
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		...
		List<LinkedHashMap<String, String>>jsonMap=excelOperation.getScenarioData(workbook, sheetName, scenarioID);
		for(LinkedHashMap<String, String> json:jsonMap) {
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_ModifySpreadOrder", scenarioID, excelOperation));
		...
		JSONObject getGroupforLiveUserRequestJsonObject=new JSONObject();
			...
			for(LinkedHashMap<String, String> jsons:jsonMaps) {
				for(LinkedHashMap<String, String> jsons1:jsonMaps1) {
					...
					getGroupforLiveUserRequestJsonObject.put("ProductType", jsons.get("ProductType"));
					getGroupforLiveUserRequestJsonObject.put("Action", jsons.get("Action"));
					getGroupforLiveUserRequestJsonObject.put("OrderDuration", jsons.get("OrderDuration"));
					getGroupforLiveUserRequestJsonObject.put("Quantity", jsons.get("Quantity"));
					getGroupforLiveUserRequestJsonObject.put("TotalPrice", jsons.get("TotalPrice"));
					getGroupforLiveUserRequestJsonObject.put("OrderID", jsons.get("OrderID"));
					getGroupforLiveUserRequestJsonObject.put("Leg1ExchangeSegment", jsons.get("Leg1ExchangeSegment"));
					getGroupforLiveUserRequestJsonObject.put("Leg1ExchangeInstrumentID", jsons.get("Leg1ExchangeInstrumentID"));
					getGroupforLiveUserRequestJsonObject.put("Leg2ExchangeSegment", jsons.get("Leg2ExchangeSegment"));
					getGroupforLiveUserRequestJsonObject.put("Leg2ExchangeInstrumentID", jsons.get("Leg2ExchangeInstrumentID"));
					getGroupforLiveUserRequestJsonObject.put("SpreadExchangeInstrumentID", jsons.get("SpreadExchangeInstrumentID"));				
	 ...
	 */
	public void modifySpreadOrder(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.Modify_SpreadOrderr.getUrl();
