...
/**
 * The Class GetSymbolforDefaultGroup.
 */
public class GetSymbolforDefaultGroup {
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			Reporter.log("<b><font size=4 color=green>Get Symbol for Default Group</font></b>");
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
		...
		List<LinkedHashMap<String, String>>jsonMap=excelOperation.getScenarioData(workbook, sheetName, scenarioID);
		for(LinkedHashMap<String, String> json:jsonMap) {
			getSymbolforDefaultGroupRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_WatchListModuleData", scenarioID, excelOperation));
	...
	private Object generateGetSymbolforDefaultGroupDataJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
		...
		List<LinkedHashMap<String, String>>jsonMap=excelOperation.getScenarioData(workbook, sheetName, scenarioID);
		for(LinkedHashMap<String, String> json:jsonMap) {
			getSymbolforDefaultGroupRequestJsonObject.put("wlID", json.get("wlID"));
		}
		return getSymbolforDefaultGroupRequestJsonObject;
	}
	 ...
	 */
	public void getSymbolforDefaultGroup(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.WatchList_GetSymbolforDefaultGroup.getUrl();
	 ...
	 * @throws Exception the exception
	 */
	public void invalidGetSymbolforDefaultGroup(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.WatchList_GetSymbolforDefaultGroup.getUrl();
