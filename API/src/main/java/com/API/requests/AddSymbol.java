...
public class AddSymbol {
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			Reporter.log("<b><font size=4 color=green>Add Symbol</font></b>");
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
		...
		List<LinkedHashMap<String, String>>jsonMap=excelOperation.getScenarioData(workbook, sheetName, scenarioID);
		for(LinkedHashMap<String, String> json:jsonMap) {
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_AddSymbolData", scenarioID, excelOperation));
	...
	private Object generateGetSymbolforDefaultGroupDataJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
		...
		for(LinkedHashMap<String, String> json:jsonMap) {
			...
			getGroupforLiveUserRequestJsonObject.put("tradingSymbol", json.get("tradingSymbol"));
			getGroupforLiveUserRequestJsonObject.put("exchange", json.get("exchange"));
		}
		return getGroupforLiveUserRequestJsonObject;
	}
	 ...
	 */
	public void addSymbol(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.WatchList_AddSymbol.getUrl();
			...
			if(infoid.equalsIgnoreCase("0")) {
				Reporter.log("<b>Symbol Added Successfully</b>");
	 ...
	 * @throws Exception the exception
	 */
	public void addInvalidSymbol(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.WatchList_AddSymbol.getUrl();
			...
			if(!infoid.equalsIgnoreCase("0")) {
				...
				String infomsg=  ((JsonObject) jsonObject.get("response")).get("infoMsg").toString().replace("\"", "");
				Reporter.log("<b>Message From Response is--></b>"+"<b>"+infomsg+"</b>");
				Assert.assertEquals("ENW0017", infoid, "Incorrect Parameters");
			}	
		}
		catch(Exception e) {
			e.printStackTrace();	
