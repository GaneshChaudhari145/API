...
/**
 * The Class PaymentTransactionHistoryGenerator.
 */
public class PaymentTransactionHistoryGenerator {
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			parentJsonObject.put("echo",generateGetforPaymentTransactionDefaultGroupEchoJSON(workbook, "DT_EchoEntity", scenarioID, excelOperation));
			...
			Reporter.log("<b><font size=4 color=green> Payment Transaction History Generator </font></b>");
	...
	/**
	 * Generate getfor payment transaction default group echo JSON.
	 *
	 * @param workbook the workbook
	 * @param sheetName the sheet name
	 * @param scenarioID the scenario ID
	 ...
	 * @return the object
	 */
	private Object generateGetforPaymentTransactionDefaultGroupEchoJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		...
		for(LinkedHashMap<String, String> json:jsonMap) {
			...
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_PaymentTransactionHistoryGen", scenarioID, excelOperation));
		...
		JSONObject getGroupforLiveUserRequestJsonObject=new JSONObject();
			...
			for(LinkedHashMap<String, String> jsons:jsonMaps) 
			{
				for(LinkedHashMap<String, String> jsons1:jsonMaps1) 
				{
					...
					getGroupforLiveUserRequestJsonObject.put("TransactionType", jsons.get("TransactionType"));
					...
					getGroupforLiveUserRequestJsonObject.put("FromDate", jsons.get("FromDate"));
					getGroupforLiveUserRequestJsonObject.put("ToDate", jsons.get("ToDate"));	
	      }
		}
		return getGroupforLiveUserRequestJsonObject;
	}
	 ...
	 */
	public void GetPaymentTransactionHistoryGenerator(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.paymentTransaction_HistoryGenerator.getUrl();
