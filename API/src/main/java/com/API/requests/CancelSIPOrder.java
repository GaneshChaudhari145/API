...
/**
 * The Class CancelSIPOrder.
 */
public class CancelSIPOrder 
{
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			parentJsonObject.put("echo",generateGetforCancelSIPOrderDefaultGroupEchoJSON(workbook, "DT_EchoEntity", scenarioID, excelOperation));
			...
			Reporter.log("<b><font size=4 color=green>Cancel SIP Order</font></b>");
	...
	/**
	 * Generate getfor cancel SIP order default group echo JSON.
	 *
	 * @param workbook the workbook
	 * @param sheetName the sheet name
	 * @param scenarioID the scenario ID
	 ...
	 * @return the object
	 */
	private Object generateGetforCancelSIPOrderDefaultGroupEchoJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
		...
		for(LinkedHashMap<String, String> json:jsonMap) {
			...
			//getGroupforLiveUserEchoJsonObject.toString();
		}
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		...
		for(LinkedHashMap<String, String> json:jsonMap) {
			...
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_CancelSIPOrder", scenarioID, excelOperation));
		...
		JSONObject getGroupforLiveUserRequestJsonObject=new JSONObject();
			...
			for(LinkedHashMap<String, String> jsons:jsonMaps) {
				for(LinkedHashMap<String, String> jsons1:jsonMaps1) {
					...
					//getGroupforLiveUserRequestJsonObject.put("sipInstructionID",GetsipInstructionIDJSON(workbook, "DT_CancelSIPOrder", scenarioID, excelOperation));
	 ...
	 */
	public void GetcancelSIPOrder(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.Cancel_SIPOrder.getUrl();
