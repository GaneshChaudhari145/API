...
/**
 * The Class PaymentGatewayStatusGenerator.
 */
public class PaymentGatewayStatusGenerator
{
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			parentJsonObject.put("echo",generateGetforPaymentGatewayDefaultGroupEchoJSON(workbook, "DT_EchoEntity", scenarioID, excelOperation));
			...
			Reporter.log("<b><font size=4 color=green> Payment Gateway Status Generator </font></b>");
	...
	/**
	 * Generate getfor payment gateway default group echo JSON.
	 *
	 * @param workbook the workbook
	 * @param sheetName the sheet name
	 * @param scenarioID the scenario ID
	 ...
	 * @return the object
	 */
	private Object generateGetforPaymentGatewayDefaultGroupEchoJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		...
		for(LinkedHashMap<String, String> json:jsonMap) {
			...
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_PaymentGatewayStatusGeneratr", scenarioID, excelOperation));
		...
		JSONObject getGroupforLiveUserRequestJsonObject=new JSONObject();
			...
			for(LinkedHashMap<String, String> jsons:jsonMaps) 
			{
				for(LinkedHashMap<String, String> jsons1:jsonMaps1)
				{
					...
					getGroupforLiveUserRequestJsonObject.put("PaymentGatewayMessage", generateMsgDetails(workbook, "DT_PaymentGatewayStatusGeneratr", scenarioID, excelOperation));					
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private Object generateMsgDetails(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		JSONObject getmsgdetails=new JSONObject();
		List<LinkedHashMap<String, String>>jsonMap=excelOperation.getScenarioData(workbook, sheetName, scenarioID);
		for(LinkedHashMap<String, String> jsonMsg:jsonMap) {
			//here CancelSIPOrder Data Want from Sheet
			//getmsgdetails.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_PaymentGatewayStatusGeneratr", scenarioID, excelOperation));
			getmsgdetails.put("Msg", jsonMsg.get("Msg"));
			getmsgdetails.put("HidRequestId", jsonMsg.get("HidRequestId"));
			getmsgdetails.put("HidOperation", jsonMsg.get("HidOperation"));
		}
		return getmsgdetails;
	}
	
	/**
	 * Gets the payment gateway status generator.
	 *
	 * @param workbook the workbook
	 * @param sheetName the sheet name
	 * @param excelOperation the excel operation
	 ...
	 */
	public void GetPaymentGatewayStatusGenerator(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.PaymentGateway_StatusGenerator.getUrl();
