...
/**
 * The Class OptionCaculator.
 */
public class OptionCaculator {
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			Reporter.log("<b><font size=4 color=green>option Calculator</font></b>");
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		...
		List<LinkedHashMap<String, String>>jsonMap=excelOperation.getScenarioData(workbook, sheetName, scenarioID);
		for(LinkedHashMap<String, String> json:jsonMap) {
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_OptionCalculator", scenarioID, excelOperation));
		...
		JSONObject getGroupforLiveUserRequestJsonObject=new JSONObject();
			...
			for(LinkedHashMap<String, String> jsons:jsonMaps) {
				for(LinkedHashMap<String, String> jsons1:jsonMaps1) {
					...
					getGroupforLiveUserRequestJsonObject.put("actualMarketValue", jsons.get("actualMarketValue"));
					getGroupforLiveUserRequestJsonObject.put("divindendRupees", jsons.get("divindendRupees"));
					getGroupforLiveUserRequestJsonObject.put("stockPrice", jsons.get("stockPrice"));
					getGroupforLiveUserRequestJsonObject.put("interestRate", jsons.get("interestRate"));
					getGroupforLiveUserRequestJsonObject.put("volatilityPercentage", jsons.get("volatilityPercentage"));
					getGroupforLiveUserRequestJsonObject.put("maturityDaysInYrs", jsons.get("maturityDaysInYrs"));
					getGroupforLiveUserRequestJsonObject.put("optionCalculationPriceModel", jsons.get("optionCalculationPriceModel"));
					getGroupforLiveUserRequestJsonObject.put("optionCalculationMode", jsons.get("optionCalculationMode"));
	 ...
	 */
	public void executeoptionCaculator(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.Option_Calculator.getUrl();
